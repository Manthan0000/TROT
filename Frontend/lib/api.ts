...
export async function patchJson<T>(path: string, body: unknown, extraHeaders: Record<string, string> = {}): Promise<Response> {
  const base = resolveApiBase();
  try {
    // eslint-disable-next-line no-console
    console.log(`API PATCH -> ${base}${path}`);
  } catch {}
  try {
    return await fetchWithTimeout(`${base}${path}`, {
      method: "PATCH",
...
export async function deleteJson(path: string, extraHeaders: Record<string, string> = {}): Promise<Response> {
  const base = resolveApiBase();
  try {
    // eslint-disable-next-line no-console
