...
export default function Settings() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {/* Dark Mode Toggle */}
      <View style={[styles.themeSection, { backgroundColor: theme.surface }]}>
      ...
      {sections.map((sec) => (
        <TouchableOpacity key={sec.title} style={[styles.item, { backgroundColor: theme.surface }]} onPress={() => router.push(sec.route)}>
          <Text style={[styles.itemText, { color: theme.text }]}>{sec.title}</Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 26, fontWeight: "bold", color: "#0f172a", marginBottom: 20 },
  themeSection: {
    flexDirection: "row",
